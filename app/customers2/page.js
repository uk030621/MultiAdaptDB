...
export default function Customers() {
  ...
  useEffect(() => {
    fetch("/api/dbnames")
      .then((res) => res.json())
      .then((data) => {
        const dbIndex = 1; // Assuming this is for "Database 2"
  ...
  const fetchCustomers = async () => {
    ...
    const response = await fetch("/api/customers2", { method: "GET" });
    const data = await response.json();
    setCustomers(data);
  ...
  return (
    <div className="min-h-screen p-8  bg-background">
      ...
      <div className="flex items-center gap-1 mt-4 mb-4">
        ...
        {/*<h1 className=" text-3xl font-bold">Database 2 Records</h1>*/}
        {/*<Image
